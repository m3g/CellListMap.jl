...
@testitem "show methods" begin
    ...
    function test_show(
            x, s::String;
            f64 = (x1, x2) -> isapprox(x1, x2, rtol = 1.0e-3),
            i64 = (x1, x2) -> x1 == x2,
    ...
    @test test_show(CellListMap.nbatches(cl), "(2, 4)")
    @test test_show(
        CellListMap.AuxThreaded(cl), """
    ...
    @test test_show(
        InPlaceNeighborList(x = x, cutoff = 0.1, unitcell = [1, 1, 1]), """
    ...
    @test test_show(
        InPlaceNeighborList(x = x, y = x, cutoff = 0.1, unitcell = [1, 1, 1]), """
            InPlaceNeighborList with types: 
            CellListMap.CellListPair{3, Float64}
            Box{CellListMap.OrthorhombicCell, 3, Float64, Float64, 9, Float64}
            Current list buffer size: 0
