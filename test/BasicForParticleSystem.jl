@testitem "BasicForParticleSystem" begin
    ...
    @test map_pairwise!((pair, u) -> potential(pair.i, pair.j, pair.d2, u, mass), system) ≈ naive
    ...
    for arrays in [
        ...
        box = Box(uc, 0.1)
        cl = CellList(x, y, box)
        r = CellListMap.map_pairwise!((pair, r) -> r += pair.d2, 0.0, box, cl)
        @test r ≈ map_pairwise!((pair, r) -> r += pair.d2, system)
...
@testitem "ParticleSystem - resize output" begin
    ...
    map_pairwise!((pair, f) -> force(pair.x, pair.y, pair.i, pair.j, pair.d2, f), sys)
...
@testitem "ParticleSystem - update_lists" begin
    ...
    for unitcell in [[1, 1, 1], nothing]
        ...
        r = CellListMap.map_pairwise!((pair, r) -> r += pair.d, 0.0, box, cl)
        @test r ≈ map_pairwise!((pair, r) -> r += pair.d, system; update_lists=false)
        ...
        r = CellListMap.map_pairwise!((pair, r) -> r += pair.d2, 0.0, box, cl)
        system.xpositions .= x
        @test r ≈ map_pairwise!((pair, r) -> r += pair.d2, system; update_lists=true)
        ...
        # compute a different property, without updating lists
        r = CellListMap.map_pairwise!((pair, r) -> r += 2*pair.d2, 0.0, box, cl)
        @test r ≈ map_pairwise!((pair, r) -> r += 2*pair.d2, system; update_lists=false)
...
@testitem "ParticleSystem parallelization" begin
    ...
    @test map_pairwise!((pair, avg_dx) -> f(pair.x, pair.y, avg_dx), system) ≈ naive
...
@testitem "ParticleSystem updating lists" begin
    ...
    system.parallel = false
    @test map_pairwise!((pair, avg_dx) -> f(pair.x, pair.y, avg_dx), system) ≈ new_naive
    system.parallel = true
    @test map_pairwise!((pair, avg_dx) -> f(pair.x, pair.y, avg_dx), system) ≈ new_naive
    ...
    new_val = CellListMap.map_pairwise!((pair, avg_dx) -> f(pair.x, pair.y, avg_dx), 0.0, new_box, new_cl)
    ...
    system.parallel = false
    @test map_pairwise!((pair, avg_dx) -> f(pair.x, pair.y, avg_dx), system) ≈ new_val
    system.parallel = true
    @test map_pairwise!((pair, avg_dx) -> f(pair.x, pair.y, avg_dx), system) ≈ new_val
    ...
    new_val = CellListMap.map_pairwise!((pair, avg_dx) -> f(pair.x, pair.y, avg_dx), 0.0, new_box, new_cl, parallel=true)
    system = ParticleSystem(
...
@testitem "ParticleSystem applications" begin
    ...
    @test naive == map_pairwise!((pair, hist) -> build_histogram!(pair.d2, hist), system)
    ...
    mass = rand(N)
    naive = CellListMap.map_pairwise!((pair, u) -> potential(pair.i, pair.j, pair.d2, u, mass), 0.0, box, cl)
    ...
    naive = CellListMap.map_pairwise!((pair, u) -> u += pair.d2, 0.0, box, cl)
    @test map_pairwise!((pair, u) -> u += pair.d2, system; update_lists=false) ≈ naive
    ...
    naive = CellListMap.map_pairwise!((pair, forces) -> calc_forces!(pair.x, pair.y, pair.i, pair.j, pair.d2, mass, forces), copy(forces), box, cl)
    system = ParticleSystem(xpositions=x, cutoff=cutoff, unitcell=sides, output=forces)
    @test map_pairwise!((pair, forces) -> calc_forces!(pair.x, pair.y, pair.i, pair.j, pair.d2, mass, forces), system) ≈ naive
