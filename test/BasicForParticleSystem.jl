@testitem "BasicForParticleSystem" begin
    ...
    @test foreachneighbor!((pair, u) -> potential(pair.i, pair.j, pair.d2, u, mass), system) ≈ naive
    ...
    for arrays in [
        ...
        box = Box(uc, 0.1)
        cl = CellList(x, y, box)
        r = CellListMap.foreachneighbor!((pair, r) -> r += pair.d2, 0.0, box, cl)
        @test r ≈ foreachneighbor!((pair, r) -> r += pair.d2, system)
...
@testitem "ParticleSystem - resize output" begin
    ...
    foreachneighbor!((pair, f) -> force(pair.x, pair.y, pair.i, pair.j, pair.d2, f), sys)
...
@testitem "ParticleSystem - update_lists" begin
    ...
    for unitcell in [[1, 1, 1], nothing]
        ...
        r = CellListMap.foreachneighbor!((pair, r) -> r += pair.d, 0.0, box, cl)
        @test r ≈ foreachneighbor!((pair, r) -> r += pair.d, system; update_lists=false)
        ...
        r = CellListMap.foreachneighbor!((pair, r) -> r += pair.d2, 0.0, box, cl)
        system.xpositions .= x
        @test r ≈ foreachneighbor!((pair, r) -> r += pair.d2, system; update_lists=true)
        ...
        # compute a different property, without updating lists
        r = CellListMap.foreachneighbor!((pair, r) -> r += 2*pair.d2, 0.0, box, cl)
        @test r ≈ foreachneighbor!((pair, r) -> r += 2*pair.d2, system; update_lists=false)
...
@testitem "cross_x_vs_sys" begin
    ...
    for uc in (nothing, [1,1,1], [1 0.2 0; 0.2 1.0 0; 0 0 1 ]), parallel in (false, true)
        ...
        @test naive ≈ foreachneighbor!(f, y, sys; update_lists=false)
        @test naive ≈ foreachneighbor!(f, y, sys; update_lists=true)
        ...
        @test naive ≈ foreachneighbor!(f, ymat, sys; update_lists=false)
        @test naive ≈ foreachneighbor!(f, ymat, sys; update_lists=true)
    ...
    cl = CellList(x, box)
    naive = CellListMap.map_naive!(f, 0.0, x, y, box)
    @test naive ≈ foreachneighbor!(f, 0.0, box, y, cl; output_threaded=nothing, parallel=true)
...
@testitem "ParticleSystem parallelization" begin
    ...
    @test foreachneighbor!((pair, avg_dx) -> f(pair.x, pair.y, avg_dx), system) ≈ naive
...
@testitem "ParticleSystem updating lists" begin
    ...
    system.parallel = false
    @test foreachneighbor!((pair, avg_dx) -> f(pair.x, pair.y, avg_dx), system) ≈ new_naive
    system.parallel = true
    @test foreachneighbor!((pair, avg_dx) -> f(pair.x, pair.y, avg_dx), system) ≈ new_naive
    ...
    new_val = CellListMap.foreachneighbor!((pair, avg_dx) -> f(pair.x, pair.y, avg_dx), 0.0, new_box, new_cl)
    ...
    system.parallel = false
    @test foreachneighbor!((pair, avg_dx) -> f(pair.x, pair.y, avg_dx), system) ≈ new_val
    system.parallel = true
    @test foreachneighbor!((pair, avg_dx) -> f(pair.x, pair.y, avg_dx), system) ≈ new_val
    ...
    new_val = CellListMap.foreachneighbor!((pair, avg_dx) -> f(pair.x, pair.y, avg_dx), 0.0, new_box, new_cl, parallel=true)
    system = ParticleSystem(
...
@testitem "ParticleSystem applications" begin
    ...
    @test naive == foreachneighbor!((pair, hist) -> build_histogram!(pair.d2, hist), system)
    ...
    mass = rand(N)
    naive = CellListMap.foreachneighbor!((pair, u) -> potential(pair.i, pair.j, pair.d2, u, mass), 0.0, box, cl)
    ...
    naive = CellListMap.foreachneighbor!((pair, u) -> u += pair.d2, 0.0, box, cl)
    @test foreachneighbor!((pair, u) -> u += pair.d2, system; update_lists=false) ≈ naive
    ...
    naive = CellListMap.foreachneighbor!((pair, forces) -> calc_forces!(pair.x, pair.y, pair.i, pair.j, pair.d2, mass, forces), copy(forces), box, cl)
    system = ParticleSystem(xpositions=x, cutoff=cutoff, unitcell=sides, output=forces)
    @test foreachneighbor!((pair, forces) -> calc_forces!(pair.x, pair.y, pair.i, pair.j, pair.d2, mass, forces), system) ≈ naive
