...
#
# loops over the particles of a neighbour cell, for 
# a LargeHighDensitySystem 
#
# all efforts are made to not run anything on unnecessary pairs 
# of particles, this includes sorting the projections of the 
# particles on the vector connecting cell centers
#
function cell_output!(
  f,
  box,
  icell,
  cl::CellList{LargeHighDensitySystem,N,T},
  output,
  jc_cartesian
) where {N,T}
  @unpack cutoff, cutoff_sq = box
  ...
  sort!(pp, by=el->el.xproj, alg=QuickSort)
