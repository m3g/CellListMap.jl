...
The returning array contains tuples with the index of the first particle, the second particle, and their distance.
...
262.141 ms (4 allocs: 958.750 MiB, 24.69% gc time, without a warmup)
```

Note that the initial buffer length has size `0`. The first time the neighbor lists are computed, the list will be allocated. Subsequent call of `neighborlist!` can be completelyl non-allocating, if the number of pairs does not increase: 
