...
For these cases, construct a `ParticleSystem` for the *reference* set (the one whose cell list you want to keep), then pass the *other* set as a plain array to `foreachneighbor`:
...
```@example mind
foreachneighbor(minimum_distance, xpositions, ysystem)
```
...
foreachneighbor(minimum_distance, xpositions, ysystem; update_lists=false)
```
...
function two_set_celllist(xpositions, ypositions)
   system = ParticleSystem(
    ...
    return foreachneighbor(minimum_distance, system)
end
```
...
function one_set_celllist(xpositions, ypositions)
   system = ParticleSystem(
    ...
    return foreachneighbor(minimum_distance, xpositions, system)
end
```
