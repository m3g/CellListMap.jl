...
```
and this function is passed directly to `foreachneighbor`:
```julia
u = foreachneighbor(energy, system)
```
...
u = foreachneighbor((pair, u) -> energy(pair, u, masses), system)
```
...
of particles. This function will be called for every pair of particles within the cutoff, automatically, in the `foreachneighbor` call.
